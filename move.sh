...
for contest_type in $contest_types; do
  ...
  for contest in $contests; do
    ...
    for task in $tasks; do
      ...
      mv $contest_type/$contest/$task/src/main/java/solver/Main.java $contest_type/$contest/$task/src/main/java/$contest_type/$contest/$task/Main.java
      sed -i '1,1d' $contest_type/$contest/$task/src/main/java/$contest_type/$contest/$task/Main.java
      echo "package $contest_type.$contest.$task;" > $contest_type/$contest/$task/src/main/java/$contest_type/$contest/$task/Main.java_
      ...
      mv $contest_type/$contest/$task/src/test/java/solver/MainTest.java $contest_type/$contest/$task/src/test/java/$contest_type/$contest/$task/MainTest.java
