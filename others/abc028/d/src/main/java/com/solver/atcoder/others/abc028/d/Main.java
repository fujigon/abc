package com.solver.atcoder.others.abc028.d;
...
public class Main {
  ...
  static void solve(InputStream is, PrintStream os) {
    ...
    // 1 different patterns (smaller is the same)
    pattern += 3 * 1 * (n - k);

    // 1 different patterns (larger is the same)
    pattern += 3 * (k - 1) * 1;

    // all the same
    pattern += 1;

    os.println((double) pattern / (double) all);
  }
