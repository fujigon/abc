package com.solver.atcoder.others.abc036.c;
...
public class Main {
  ...
  static void solve(InputStream is, PrintStream os) {
    ...
    for (int i = 0; i < n; i++) {
      a.add(new Value(sc.nextInt(), i));
    }

    a.sort(Comparator.comparingInt(Value::getValue));

    int compress = 0;
    int last = a.get(0).getValue();

    for (int i = 0; i < n; i++) {
      if (a.get(i).value != last) {
        compress++;
      }
      a.get(i).compress = compress;
      last = a.get(i).getValue();
    }

    a.sort(Comparator.comparingInt(Value::getIndex));

    for (int i = 0; i < n; i++) {
      os.println(a.get(i).compress);
    }
  }
