package com.solver.atcoder.others.agc014.b;
...
public class Main {
  ...
  static void solve(InputStream is, PrintStream os) {
    ...
    Set<Integer> leaves = dfs(0, new HashSet<>(), graph);
    if (graph.get(1).equals(leaves)) {
      os.println("YES");
    } else {
      os.println("NO");
    }
  ...
  private static Set<Integer> dfs(int start, Set<Integer> visited, Map<Integer, Set<Integer>> graph) {
    Set<Integer> leaves = new HashSet<>();
    visited.add(start);
    Set<Integer> children = new HashSet<>(graph.get(start));
    ...
    for (int child : children) {
      leaves.addAll(dfs(child, visited, graph));
    }
