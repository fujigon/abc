package com.solver.atcoder.others.arc008.b;
...
public class Main {
  ...
  static void solve(InputStream is, PrintStream os) {
    ...
    Map<Character, Integer> nameHist = new HashMap<>();
    for (int i = 0; i < n; i++) {
      nameHist.put(name.charAt(i), nameHist.getOrDefault(name.charAt(i), 0) + 1);
    }

    Map<Character, Integer> kitHist = new HashMap<>();
    for (int i = 0; i < m; i++) {
      kitHist.put(kit.charAt(i), kitHist.getOrDefault(kit.charAt(i), 0) + 1);
    }

    int max = 0;
    for (char c : nameHist.keySet()) {
      if (!kitHist.containsKey(c)) {
        os.println(-1);
        return;
      }
      max = Math.max(max, (nameHist.get(c) + kitHist.get(c) - 1) / kitHist.get(c));
    }
    os.println(max);
  }
