...
public class Main {
  ...
  static void solve(InputStream is, PrintStream os) {
    ...
    for (int i = 0; i < m; i++) {
      int x = sc.nextInt() - 1;
      int y = sc.nextInt() - 1;
      Set<Integer> setX = relations.getOrDefault(x, new HashSet<>());
      setX.add(y);
      relations.put(x, setX);

      Set<Integer> setY = relations.getOrDefault(y, new HashSet<>());
      setY.add(x);
      relations.put(y, setY);
    }
    ...
    for (int i = 0; i < (1 << n); i++) {
      ...
      boolean ok = true;
      for (Integer candidate : party) {
        ok = ok & relations.get(candidate).containsAll(party);
      }
      if (ok) {
        ans = Math.max(ans, party.size());
      }
    }
    os.println(ans);
  }
