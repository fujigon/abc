...
public class Main {
  ...
  static void solve(InputStream is, PrintStream os) {
    ...
    Solver solver = null;
    if (k == 1) {
      solver = new Solver1(n, k);
    } else if (k == 2) {
      solver = new Solver2(n, k);
    } else if (k == 3) {
      solver = new Solver3(n, k);
    }

    long ans = 0;
    int bits = (int) Math.pow(9, k);
    for (int i = 0; i < bits; i++) {
      List<Integer> digits = new ArrayList<>(k);
      for (int b = 0; b < k; b++) {
        int base = (int) Math.pow(9, b);
        digits.add((i / base) % 9 + 1);
      }
      ans += solver.count(digits);
    }
    os.println(ans);
  }

  private static class Trailing {

    int first;
    int numOfZero;

    public Trailing(int first, int numOfZero) {
      this.first = first;
  ...
  private interface Solver {
    ...
    long count(List<Integer> nonzero);

    default String generate(List<Trailing> trailingList) {
      StringBuilder p = new StringBuilder();
      for (Trailing t : trailingList) {
        p.append(t.first);
        for (int i = 0; i < t.numOfZero; i++) {
          p.append('0');
        }
      }
      return p.toString();
  ...
  private static class Solver1 implements Solver {
    ...
    private Solver1(String n, int k) {
      this.n = n;
      this.k = k;
      this.bound = new BigInteger(n);
    }

    @Override
    public long count(List<Integer> nonzero) {
      int digits = n.length();
      int zeros = digits - 1;

      List<Trailing> trailingList = new ArrayList<>(1);
      trailingList.add(new Trailing(nonzero.get(0), zeros));

      long pattern = 1;
      for (int i = 0; i < trailingList.size() - 1; i++) {
        pattern *= trailingList.get(i).numOfZero + 1;
      }
      if (new BigInteger(generate(trailingList)).compareTo(bound) <= 0) {
        pattern *= trailingList.get(trailingList.size() - 1).numOfZero + 1;
      } else {
        pattern *= trailingList.get(trailingList.size() - 1).numOfZero;
      }
      return pattern;
    }
  }

  private static class Solver2 implements Solver {

    private final String n;
    private final BigInteger bound;
    private final int k;

    private Solver2(String n, int k) {
      this.n = n;
      this.k = k;
      this.bound = new BigInteger(n);
    }
    ...
    public long count(List<Integer> nonzero) {
      ...
      long ans = 0;
      for (int zeros1 = 0; zeros1 <= zeros; zeros1++) {
        int zeros2 = zeros - zeros1;
        List<Trailing> trailingList = new ArrayList<>();
        trailingList.add(new Trailing(nonzero.get(0), zeros1));
        trailingList.add(new Trailing(nonzero.get(1), zeros2));
  ...
  private static class Solver3 implements Solver {

    private final String n;
    private final BigInteger bound;
    private final int k;

    private Solver3(String n, int k) {
      this.n = n;
      this.k = k;
      this.bound = new BigInteger(n);
    }
    ...
    public long count(List<Integer> nonzero) {
      ...
      long ans = 0;
      for (int zeros1 = 0; zeros1 <= zeros; zeros1++) {
        for (int zeros2 = 0; zeros2 <= zeros - zeros1; zeros2++) {
          List<Trailing> trailingList = new ArrayList<>();
          int zeros3 = zeros - zeros1 - zeros2;
          trailingList.add(new Trailing(nonzero.get(0), zeros1));
          trailingList.add(new Trailing(nonzero.get(1), zeros2));
          trailingList.add(new Trailing(nonzero.get(2), zeros3));
          ...
          if (new BigInteger(generate(trailingList)).compareTo(bound) <= 0) {
            pattern *= trailingList.get(trailingList.size() - 1).numOfZero + 1;9
          } else {
            pattern *= trailingList.get(trailingList.size() - 1).numOfZero;
          }
          ans += pattern;
...
//    private final int k;
//
//    public Solver(String n, int k) {
//      this.n = n;
//      this.k = k;
//    }
//
//    long count(List<Trailing> trailingList) {
//      long ans = 1;
//      StringBuilder p = new StringBuilder();
//      trailingList.forEach(t -> {
//        p.append(t.first);
//        for (int i = 0; i < t.numOfZero; i++) {
//          p.append('0');
//        }
...
//      }
//      if (new BigInteger(pattern).compareTo(new BigInteger(n)) > 0) {
//        return 0;
//      }
//      for (int i = 0; i < trailingList.size(); i++) {
//        trailingList.get(i).numOfZero++;
//        ans += count(trailingList);
//        trailingList.get(i).numOfZero--;
//      }
//      memo.put(pattern, ans);
//      return ans;
//    }
//
//
