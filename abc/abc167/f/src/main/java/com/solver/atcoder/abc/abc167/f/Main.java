package com.solver.atcoder.abc.abc167.f;
...
public class Main {
  ...
  static void solve(InputStream is, PrintStream os) {
    ...
    for (int i = 0; i < n; i++) {
      operate(stack, sc.next());
    }

  }

  private static boolean operate(Deque<String> stack, String str) {
    Deque<Character> s = new ArrayDeque<>();
    boolean res = false;
    for (int j = 0; j < str.length(); j++) {
      res = operate(s, str.charAt(j)) | res;
    }
  ...
  private static boolean operate(Deque<Character> stack, char c) {
    if (c == '(') {
      stack.push('(');
      return false;
    }
