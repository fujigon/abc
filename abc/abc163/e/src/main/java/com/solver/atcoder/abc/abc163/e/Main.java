...
public class Main {
  ...
  static void solve(InputStream is, PrintStream os) {
    ...
    os.println(helper(0, n, a, cul, new long[n + 1][n + 1]));
  }

  private static long helper(int l, int r, long[] a, long[] cul, long[][] memo) {
    if (r < l) return 0;
    if (l == r) return 0;
    if (l + 1 == r) return 0;
    if (l == r - 2) return a[l + 1] + a[l];
    if (memo[l][r] != 0) return memo[l][r];
    long max = 0;
    for (int i = l + 1; i < r; i++) {
      long left = helper(l, i, a, cul, memo);
      long right = helper(i, r, a, cul, memo);
      long value = left + right;
      max = Math.max(max, value);
    }
    max = Math.max(max, helper(l, r - 1, a, cul, memo) + (cul[r - 1] - cul[l]) + a[r - 1] * (r - l));
    memo[l][r] = max;
    return max;
  }
