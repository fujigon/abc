package com.solver.atcoder.abc.abc161.f;
...
public class Main {
  ...
  static void solve(InputStream is, PrintStream os) {
    ...
    if ((long) Math.sqrt(n) * ((long) Math.sqrt(n) + 1) == n) {
      ans++;
    }
    while ((long) Math.sqrt(n) * (long)Math.sqrt(n) == n) {
      ans++;
      n /= (long) Math.sqrt(n);
    }
    os.println(ans);
  ...
  private static Set<Long> factors(long n) {
    ...
    while (factor <= bound) {
      while (n % factor == 0) {
        Set<Long> multiplied = new HashSet<>();
        for (long existing : factors) {
          multiplied.add(existing * factor);
        }
        factors.addAll(multiplied);
        n /= factor;
      }
      factor++;
    }
