package com.solver.atcoder.abc.abc155.d;
...
public class Main {
  ...
  static void solve(InputStream is, PrintStream os) {
    ...
    long[] neg = Arrays.copyOfRange(a, 0, index);
    long[] pos = Arrays.copyOfRange(a, index, a.length);

    if (k < neg.length * pos.length) { // in the negative range
      int nIndex = 0;
      int pIndex = pos.length - 1;
      int cand = n - 1;
      while (cand <= k) {
        k -= cand;
        cand -= 2;
      ...
      for (int pi = 0; pi < pIndex; pi++) {
        candidates.add(neg[nIndex] * pos[pi]);
      }
      for (int ni = nIndex + 1; ni < neg.length; ni++) {
        candidates.add(neg[ni] * pos[pIndex]);
      }
      candidates.add(neg[nIndex] * pos[pIndex]);
      Collections.sort(candidates);
      os.println(candidates.get(k));
    }

    k -= neg.length * pos.length;

    int nIndex = neg.length - 1;
    int pIndex = 0;
    int cand = (neg.length - 1) * 2 - 1 + (pos.length - 1) * 2 - 1;
    while (0 <= nIndex && pIndex < pos.length && cand <= k) {
